...
class TestClassesDoNotCountInstances(unittest.TestCase):
    """The finding the blind runs were retracted over, re-established in the
    form that survives the parser repair.

    Named per code and per file, a failure set shrinking *within one file* does
    not move at all: 7 findings, then 3, then 1 is `FLAT` twice over, and
    `flat_cycles` reaches `reviewWhenStuck`'s default rung on a ticket that is
    converging as fast as anything in this repository ever has.

    `_convergence` compares sets and nothing reads their size. That is the gap
    the *Adaptive ticket loop* entry in `docs/ROADMAP.md` argues from, and this
    is it happening to real output."""

    def setUp(self):
        recording = recorded.Recording(BLIND)
        # Subsets of one recorded step: the same real findings, fewer of them.
        # Truncating a recording keeps every line a tool wrote; typing a
    ...
    def test_but_every_cycle_after_the_first_reads_flat(self):
        verdicts = [
    ...
    def test_and_the_ticket_reaches_the_rung_the_ladder_fires_on(self):
        for detail in self.shrinking:
            recorded.cycle(self.orchestrator, self.store, self.run_id, detail)

        ticket = self.store.list_tickets(self.run_id)[0]
        self.assertEqual(ticket.flat_cycles, 2)
        self.assertGreaterEqual(
            ticket.flat_cycles, self.orchestrator.config.loop.review_when_stuck
        )
