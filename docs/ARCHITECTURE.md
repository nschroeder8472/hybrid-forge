...
Both retry edges are bounded by `maxAttempts` (default 3). Past that the ticket
parks as failed. A `BLOCKED:` does not retry at all: an underspecified spec does
...
**Triage.** A ticket routed `claude-only` is left for a human even if that stalls
the backlog. The loop is not entitled to overrule the plan to keep moving.
